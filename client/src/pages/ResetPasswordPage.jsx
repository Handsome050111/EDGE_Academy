...
const ResetPasswordPage = () => {
  ...
  useEffect(() => {
    if (!token) {
      setError('No password reset token provided. Please use the link sent to your email.');
      setLoading(false);
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate('/login', {
          state: {
            activatedEmail: res.data?.email || userInfo?.email,
            resetSuccess: true,
          },
        });
      }, 2200);
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to reset password. Please request a new reset link.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="h-screen w-screen overflow-hidden bg-white flex flex-col lg:flex-row font-sans">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-[#062452] p-8 lg:p-14 xl:p-16 flex-col justify-between text-white relative overflow-hidden shrink-0">
        ...
        <div className="relative z-10 max-w-md my-auto">
          <div className="inline-flex items-center gap-2 bg-blue-400/10 border border-blue-300/20 text-blue-200 text-xs px-3.5 py-1.5 rounded-full font-semibold mb-4">
          ...
          <p className="text-sm lg:text-base text-blue-200/80 leading-relaxed font-normal mt-3 mb-6">
            Choose a new, secure corporate password to regain access to your field engineering tracks and dashboard.
          </p>
        ...
        <div className="max-w-[420px] w-full mx-auto my-auto py-6 lg:py-0">
          ...
          {loading ? (
            <div className="py-16 text-center space-y-4">
              ...
              <p className="text-sm font-semibold text-slate-600">Verifying reset token...</p>
            </div>
          ) : success ? (
            <div className="text-center py-8 space-y-5">
              ...
              <button
                onClick={() =>
                  navigate('/login', {
                    state: { activatedEmail: userInfo?.email, resetSuccess: true },
                  })
                }
            ...
            <div>
              ...
              <div className="mb-8">
                ...
                <p className="text-sm text-slate-500 font-normal">
                  {userInfo?.email ? (
                    <>
                      Updating credentials for <span className="font-semibold text-slate-700">{userInfo.email}</span>
                    </>
                  ) : (
                    'Enter your new corporate password below'
                  )}
                </p>
              </div>
