  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {/* Sub-view Toggle */}
        <div className="grid grid-cols-2 gap-1 w-full sm:w-auto sm:inline-flex bg-slate-100 p-1 rounded-xl border border-slate-200 shrink-0">
          <button
            type="button"
            onClick={() => setActiveSubTab('dispatch')}
            className={`w-full sm:w-auto px-3 py-2 sm:px-3.5 sm:py-1.5 rounded-lg text-xs font-medium transition cursor-pointer flex items-center justify-center gap-1.5 whitespace-nowrap ${
          ...
          >
            ...
            <span className="hidden sm:inline">Dispatch Assignments</span>
          ...
          >
            ...
            <span className="hidden sm:inline">View All Assignments ({assignmentsList.length})</span>
            <span className="sm:hidden">All Assignments ({assignmentsList.length})</span>
          </button>
        </div>
      </div>
                      ...
                      return (
                        <tr key={item._id} className="hover:bg-slate-50/70 transition">
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-xs font-normal text-slate-500 truncate">{eng.email || 'No email'}</p>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3.5">
                            <div>
                              <p className="text-sm font-semibold text-slate-900">{mod.title || 'Training Module'}</p>
                          ...
                          <td className="px-4 py-3.5">
                            <p className="text-sm font-medium text-slate-800">{assigner.fullName || assigner.full_name || 'Administrator'}</p>
                            <p className="text-xs font-normal text-slate-500 capitalize">{assigner.role || 'Admin'}</p>
                          </td>
                          ...
                          <td className="px-4 py-3.5">
                            {item.deadline_at ? (
                              <span className={`text-xs font-medium ${isOverdue ? 'text-rose-600 font-bold' : 'text-slate-700'}`}>
                                {new Date(item.deadline_at).toLocaleDateString()}
                              </span>
                            ) : (
                              <span className="text-slate-400 text-xs font-normal">No deadline</span>
                            )}
                          </td>
